    ...
    def create(self,  request, *args, **kwargs):
        ...
        for loan in Loan.objects.filter(book_copy=book_copy):
            ...
            if loan.devolution_date is None:
                return Response({"detail" : "This copy is already loaned"}, status=status.HTTP_400_BAD_REQUEST)
        ...
        for loan in Loan.objects.filter(user=user):
            ...
            if loan.devolution_date is None and today_date > expiration_date :
                return Response({"detail" : "The user has not returned expired loans"}, status=status.HTTP_403_FORBIDDEN)
            ...
            if loan.devolution_date:
                if loan.devolution_date > expiration_date:
                    ...
                    if today_date < block_date:
                        return Response({"detail" : "The user is temporarily blocked from making loans"}, status=status.HTTP_403_FORBIDDEN)
